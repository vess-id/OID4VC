...
export const assertValidRequestObjectOpts = (opts: RequestObjectOpts<ClaimPayloadCommonOpts>, checkRequestObject: boolean) => {
  if (!opts) {
  ...
  } else if (opts.passBy !== PassBy.REFERENCE && opts.passBy !== PassBy.VALUE && opts.passBy !== PassBy.NONE) {
    ...
    } else if (checkRequestObject && opts.passBy !== PassBy.NONE) {
      // PassBy.NONE doesn't require payload in request object
